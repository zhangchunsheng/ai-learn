...
logger = logging.getLogger('Training a chinese char recognition');
...
tf.app.flags.DEFINE_integer('max_steps', 10001, "the max training steps")
...
def build_graph(top_k):
    ...
    images = tf.placeholder(dtype=tf.float32, shape=[None, 60, 60, 1], name='image_batch');
    labels = tf.placeholder(dtype=tf.int64, shape=[None], name='label_batch');

    conv_1 = slim.conv2d(images, 60, [3, 3], 1, padding='SAME', scope='conv1');
    max_pool_1 = slim.max_pool2d(conv_1, [2, 2], [2, 2], padding='SAME');
    conv_2 = slim.conv2d(max_pool_1, 120, [3, 3], padding='SAME', scope='conv2');
    max_pool_2 = slim.max_pool2d(conv_2, [2, 2], [2, 2], padding='SAME');
    conv_3 = slim.conv2d(max_pool_2, 240, [3, 3], padding='SAME', scope='conv3');
    max_pool_3 = slim.max_pool2d(conv_3, [2, 2], [2, 2], padding='SAME');

    flatten = slim.flatten(max_pool_3);
    fc1 = slim.fully_connected(slim.dropout(flatten, keep_prob), 20, activation_fn=tf.nn.tanh, scope='fc1');
    logits = slim.fully_connected(slim.dropout(fc1, keep_prob), 20, activation_fn=None, scope='fc2');
        ...
        try:
            while not coord.should_stop():
                ...
                if step % FLAGS.save_steps == 1:
                    ...
                    saver.save(sess, os.path.join('./checkpoint/', 'chn-model'), global_step=graph['global_step']);
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                batch_labels, probs, indices, acc_1, acc_k = sess.run(
                    [graph['labels'], graph['predicted_val_top_k'], graph['predicted_index_top_k'], graph['accuracy'], graph['accuracy_top_k']],
                    feed_dict=feed_dict
                );
...
def inference(image):
    ...
    temp_image = temp_image.reshape([-1, 60, 60, 1])
    with tf.Session() as sess:
        logger.info("============Start inference=============");
        # images = tf.placeholder(dtype=tf.float32, shape=[None, 60, 60, 1])
